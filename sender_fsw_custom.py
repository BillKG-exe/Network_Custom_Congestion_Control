            ...
            try:
                ...
                if ack_id > (window_start * MESSAGE_SIZE):
                    ...
                    if(WINDOW_SIZE < SSTHRESH):
                        ...
                        # CONGESTION AVOIDANCE, GROW HALF AS SLOWLY AS SLOW START
                        WINDOW_SIZE += max((window_start - window_start_old) // 2, 1)
                ...
                # SET SSTHRESH TO HALF OF CWND
                SSTHRESH = max(WINDOW_SIZE // 2, 1)
                # RESET CWND TO HALF OF SSTHRESH
                WINDOW_SIZE = max(SSTHRESH // 2, 1)
                # double the timeout
                timeout *= 2
                udp_socket.settimeout(timeout)
